...
① UTM 创建 x86_64 VM（手工）
   • Legacy BIOS（不要 UEFI）
   • IDE 总线（UTM 4.x GUI 没暴露 SATA；IDE 与 Win10 内建 PIIX 驱动兼容）
   • e1000 网卡
...
- **同 hypervisor 优势**：UTM 用的就是 QEMU；Mac 上跑的客户机和服务器上跑的几乎完全等价（machine type / chipset / IDE / e1000 全部一致）。**这意味着客户机里看到的硬件指纹在 Mac 上和服务器上是一样的，anti-VM 检测的"训练环境 vs 部署环境"差异为零。**
...
| RAM | 4096 MiB |
| 磁盘总线 | IDE（UTM 4.x GUI 没暴露 SATA；pc-i440fx 自带 piix3-ide 控制器） |
| 网卡 model | `e1000` |
...
| 磁盘 IDE 而非 virtio | IDE | Win10 自带 PIIX 驱动；UTM 4.x GUI 不暴露 SATA；virtio 需驱动注入 |
