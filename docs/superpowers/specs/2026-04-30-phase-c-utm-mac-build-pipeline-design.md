...
│   └── c-host-export.sh             NEW   bash, 在 Mac 跑（替代原 .ps1）
│
├── lib/
...
│
└── docs/
    ├── BUILD-GUEST-ON-MAC.md        NEW   Mac (UTM) 完整手册（替代原 BUILD-GUEST-ON-WINDOWS.md）
    ├── WHY.md                       EDIT  + ADR-Phase-C（决策含 UTM 抉择）
    └── INSTALL.md                   EDIT  + 一句话指向 BUILD-GUEST-ON-MAC.md
```

### 4.1 命名约定

- `c-` 前缀（client/cuckoo）与既有 `00-99` 安装、`u00-u99` 卸载脚本不冲突
- `c-guest-prep.ps1` 名字保留 `.ps1`——它跑在 Win10 客户机内，与宿主无关
- `c-host-export.sh` 名字改成 `.sh`——它跑在 Mac shell

### 4.2 文件大小预估

- 5 个 c-stage bash 共 ~400 行
- `c-guest-prep.ps1` ~150 行（不变）
- `c-host-export.sh` ~80 行（比原 .ps1 简化：无 qemu-img convert 步骤）
- `domain-cuckoo1.xml.tmpl` ~50 行
- `BUILD-GUEST-ON-MAC.md` ~350 行（比 Windows 版省一节 VMDK 转换 + Tools 警告）
- `lib/common.sh` 增量 ~40 行

### 4.3 把 c-guest-prep.ps1 送进客户机的三选项（写在文档里）

UTM 把客户机文件分发选项与 VMware 略不同：
...
2. **临时联网 + gh-proxy 拉**——客户机临时连 NAT 网络，PowerShell `Invoke-WebRequest https://gh-proxy.com/...` 拉脚本。快但需短暂联网。
3. **UTM SPICE 共享目录**——UTM 支持 SPICE 通过 webdavs 文件夹共享，但要装 SPICE Guest Tools，与"零工具"原则冲突，**不推荐**。
...
依赖关系（与旧 spec 几乎一致，仅替换 host 侧脚本）：
...
common.sh helpers   ─┤
                     ...
                     │
c-guest-prep.ps1    ─┤
c-host-export.sh    ─┘     ← Bash 替代原 PowerShell
                     │
                     └─► BUILD-GUEST-ON-MAC.md（写在最后，引用真跑通的命令）
                         WHY.md ADR-Phase-C 追加
                         INSTALL.md / README.md 加指针
```

建议实施 plan 拆分为：
1. **基础**：config.env.sample + domain-xml.tmpl + common.sh helpers
2. **服务器侧 5 个 c-stage**（c10→c50，每个独立 commit）
3. **Makefile**：c-stage targets + import-guest 元 target + GUEST_QCOW2 校验
4. **客户机/宿主脚本**：c-guest-prep.ps1（先于 c-host-export.sh，因为后者依赖前者产出的关机 VM）
5. **文档**：BUILD-GUEST-ON-MAC.md + WHY.md ADR + 指针更新
...
| 客户机加固 | `c-guest-prep.ps1` 不变 |
| 服务器侧 5 个 c-stage | **完全不变** |
| Makefile | 不变 |
| lib/common.sh helpers | 不变 |
| domain XML 模板 | 不变 |
| 网络/IP/快照/kvm.conf 契约 | 不变 |
| 用户文档 | `BUILD-GUEST-ON-WINDOWS.md` → `BUILD-GUEST-ON-MAC.md`（省 VMDK 转换、加 UTM 设置截图说明） |
| ADR 多一行 | "抛弃 Fusion 改用 UTM" 决策记录 |
